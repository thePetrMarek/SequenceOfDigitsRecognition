...
class SequenceBiggerOutput:
    def get_name(self):
        return "sequence_bigger_output_dropout"
        ...
        with tf.name_scope("inference"):
            ...
            with tf.variable_scope("RNN"):
                for i in range(5):
                    ...
                    fc_1 = self._fully_connected(output, 576, 256)
                    dropout_output1 = tf.nn.dropout(fc_1, keep_prob)
                    relu_output1 = self._relu(dropout_output1)

                    fc_2 = self._fully_connected(relu_output1, 256, 64)
                    dropout_output2 = tf.nn.dropout(fc_2, keep_prob)
                    relu_output2 = self._relu(dropout_output2)

                    fc_3 = self._fully_connected(relu_output2, 64, 10)
                    number_logits = fc_3
